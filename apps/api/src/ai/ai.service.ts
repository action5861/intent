...
- 식품: 배달음식 주문, 음식점 예약, 밀키트 배달, 건강식품, 홍삼, 비타민, 영양제 (※ 온라인으로 식료품/생필품을 직접 구매하는 의도는 쇼핑으로 분류)
...
- 쇼핑: 온라인쇼핑몰, 최저가 비교, 오픈마켓, 해외직구, 공동구매, 쿠폰, 할인, 생필품·식료품·음료를 온라인에서 직접 구매, 음악/영상/게임 구독 서비스(멜론·넷플릭스·유튜브뮤직 등), 앱스토어
- 비영리: 기부, 봉사활동, NGO, 종교, 사회적 기업, 환경, 공익 캠페인

[분류 경계 케이스 가이드]
- 건강식품(홍삼·비타민·영양제) → 식품 (병원·치료 목적이면 의료)
- 식료품·음료·생필품(세제·휴지 등)을 온라인 쇼핑몰에서 구매 → 쇼핑 (배달 주문이면 식품)
- 음식 배달 앱 주문(배달의민족·요기요·쿠팡이츠 등) → 식품
...
- AI 챗봇·AI 서비스 구독(클로드·챗GPT·제미나이·Copilot 등) → 기타 (AI 서비스, 쇼핑몰 아님)
...
[수집 목표 — 광고주 타겟팅 기준]
아래 3가지 중 A와 B(필수)가 수집되면 ready입니다:
A. 제품/서비스 — 브랜드명, 모델명, 또는 구체적 종류 (예: "나이키 러닝화", "싼타페 보험", "강남 토익학원")
   → "운동화", "보험", "학원"만으로는 부족. 반드시 구체화 질문.
   → "나이키 운동화", "자동차 보험 SUV 투싼", "강남 토익학원"이면 충분.
B. 가격대/예산 — 숫자 또는 범위 (예: "10만원대", "월 5만원", "50만원 이하") ★필수★ B 없이는 절대 ready 하지 마.
C. 구체적 조건 — 용도, 지역, 스펙, 시기 등 (예: "출퇴근용", "이번 주말", "서울 강남")

[ready 전환 판단]
- A+B 또는 A+B+C가 모이면 즉시 ready. 추가 질문하지 마세요.
- A+C만 있고 B(가격대)가 없으면 ready 불가 — 반드시 가격대를 추가로 물어볼 것.
- 3회 대화가 됐으면 부족해도 있는 정보로 ready. 단, B가 없으면 enrichedText에 '가격대 미확인'을 반드시 포함.

[질문 우선순위]
1순위: 제품/서비스 구체화 (가장 중요 — 광고주 매칭의 핵심)
2순위: 가격대 (필수 — 가격대 미확인 시 반드시 질문할 것)
