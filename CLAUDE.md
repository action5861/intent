...
│   │   ├── rate-limit.middleware.ts         ← JWT 기반 2회/24h
...
유틸: apps/api/src/common/similarity.util.ts → calcJaccardSimilarity(), extractKeywords()
...
- rate-limit(2회/24h)과 독립적인 별도 방어층
```
...
| 미들웨어 | 기준 | 제한 |
|---|---|---|
| `rate-limit.middleware.ts` | JWT userId | 2회/24시간 (POST /api/intents) |
